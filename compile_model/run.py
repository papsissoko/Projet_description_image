...
val_images_processed =  val_images.map(preprocesser.call, num_parallel_calls=tf.data.AUTOTUNE)
...
images_val_array = np.concatenate([x for x in val_images_processed], axis=0)

text_in_ds = tokens[:len(images_array), :-1]
text_out_ds = tokens[: len(images_array),1:]

text_in_ds_val = tokens_val[:len(images_val_array):, :-1]
text_out_ds_val = tokens_val[:len(images_val_array),1:]
